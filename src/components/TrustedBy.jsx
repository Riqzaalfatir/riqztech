...
const TrustedBy = () => {
  return (
    ...
    className='flex flex-col items-center px-4 sm:px-12 lg:px-24 xl:px-40 gap-10 text-gray-700 dark:text-white/80'>
      ...
      className='font-semibold'>Mitra Kolaborasi Perusahaan</motion.h3>
      <motion.div 
      initial="hidden"
          whileInView="visible"
          transition={{stragerChildren: 0.1 }}
          viewport={{once: true}}
      ...
      className='flex items-center justify-center flex-wrap gap-10 m-4'>
        {company_logos.map((logo, index)=>(
            ...
            key={index} src={logo} alt="" className='max-h-8 w-32 sm:max-h-9 dark:drop-shadow-xl' />
        ))}
      </motion.div>
    </motion.div>
  )
